...
nombre_archivo = "C:/Users/bryan/Downloads/Procesos_Suspendidos.txt"
...
def animacion():
    print(Cursor.POS(25, 2) +"Proceso N.siguiente")
    print(Cursor.POS(74, 2) +"Proceso S.siguiente")
...
def BCP(all_process_finish,memoria,bloqueados,suspendidos,all_process,null_process,contador,mostrar,iteraciones):#El estado determina si se mostrara la tabla en ejecucion o al finalizar
  var = 15
  pos = 0
  ...
  if mostrar == "fin":
    ...
    # print(Cursor.POS(1,11+pos) + "Fernandez Roman Bryan Daniel")
      ...
      for num, process in enumerate(memoria):#Mostrar procesos en listos y ejecucion
            process,_ = operando(process,pos)
            ...
            if num == 0:
              ...
              print(Cursor.POS(var + 136, 6+pos) + Fore.BLUE + Style.BRIGHT + "Listo" + Style.RESET_ALL) #Estado   
            pos += 1
      for process in bloqueados: #Mostrar procesos bloqueados
            ...
            pos += 1
      for process in suspendidos: #Mostrar procesos suspendidos
            process,_ = operando(process,pos)
            tiempo_auxiliar_retorno = round(contador - process.tiempo_llegada,1)
            tiempo_espera_momento = round(tiempo_auxiliar_retorno - process.tiempo_transcurrido,1)
            tiempo_restante_cpu = round(process.tiempo - process.tiempo_transcurrido,1)
            ...
            print(Cursor.POS(var + 107, 6+pos),tiempo_restante_cpu) #Restante en CPU
            print(Cursor.POS(var + 121, 6+pos)+ Style.BRIGHT  + "N/A" + Style.RESET_ALL) #Bloqueado
            print(Cursor.POS(var + 136, 6+pos) + Fore.MAGENTA + Style.BRIGHT + "Suspendido" + Style.RESET_ALL) #Estado  
            pos += 1
...
def guardar_informacion(proceso):
    string =  str(proceso.id) +" "+ str(proceso.n1) +" "+ str(proceso.n2) +" "+ str(proceso.opcion) +" "+ str(proceso.finalizacion) +" "+ str(proceso.tiempo) \
    +" "+ str(proceso.tiempo_transcurrido) +" "+ str(proceso.tiempo_bloqueado) +" "+ str(proceso.tiempo_llegada) +" "+ str(proceso.tiempo_finalizacion) +" "+ \
    str(proceso.tiempo_respuesta) +" "+ str(proceso.tiempo_respuesta_bandera) +" "+ str(proceso.quantum) +" "+ str(proceso.size) +" "+ str(proceso.num_paginas_ocupadas) +  "\n"
    return string

def recrea_proceso(informacion_suspendido):
    id = int(informacion_suspendido[0])
    opcion = int(informacion_suspendido[3])
    n1 = int(informacion_suspendido[1])
    n2 = int(informacion_suspendido[2])
    finalizacion = int(informacion_suspendido[4])
    tiempo = int(informacion_suspendido[5])
    tiempo_transcurrido = float(informacion_suspendido[6])
    tiempo_bloqueado = float(informacion_suspendido[7])
    tiempo_llegada = float(informacion_suspendido[8])
    tiempo_finalizacion = float(informacion_suspendido[9])
    tiempo_respuesta = float(informacion_suspendido[10])
    tiempo_respuesta_bandera = True
    quantum = int(informacion_suspendido[12])
    size = int(informacion_suspendido[13])
    num_paginas_ocupadas = int(informacion_suspendido[14])
...
def main():
  ...
  bloqueados = []
  suspendidos = []
  procesos_suspendidos = 0
  peticion_procesos_suspendidos = False
  siguiente_proceso = 0
  siguiente_size = 0
  ...
  print(Cursor.POS(48, 2) +"Procesos suspendidos: 0")#Primera impresion de procesos, se toman los totales y se le restan 5 para la primera impresion
  ...
  while len(memoria) + len(bloqueados) + procesos_suspendidos > 0:
    borrado_Pripantalla()
    ...
    if len(memoria) == 0:#Este condicional se utiliza cuando hay n-1 procesos en bloqueados y un proceso en ejecucion, al presionar la letra de error, cae en esta condicion
      while len(memoria)==0:
        if procesos_suspendidos > 0:
          with open(nombre_archivo, 'r') as archivo:
            lineas = archivo.readlines()
          informacion_suspendido = [""] * 15#Lista que almacena los valores del proceso suspendido
          aux_informacion_suspendido = 0 #Sirve para determinar en que punto de la cadena se quedo, y continuar iterando desde ahi
          for i in range(15):
            for letra in range(aux_informacion_suspendido,len(lineas[0])):
              aux_informacion_suspendido += 1
              if lineas[0][letra] != " " and lineas[0][letra] != "\n":
                informacion_suspendido[i] = informacion_suspendido[i] + lineas[0][letra]
              else:
                break
          siguiente_proceso = int(informacion_suspendido[0])
          siguiente_size = int(informacion_suspendido[13])
        print(Cursor.POS(2, 3) +"Quantum:",quantum) #Valor del quantum
        print(Cursor.POS(2, 2) +"Procesos nuevos:    ")
        print(Cursor.POS(2, 2) +"Procesos nuevos:",aux)
        print(Cursor.POS(48, 2) +"Procesos suspendidos:    ")
        print(Cursor.POS(48, 2) +"Procesos suspendidos:",procesos_suspendidos)
        print(Cursor.POS(25, 3) +"                 ")
        print(Cursor.POS(34, 3) +"                 ")
        print(Cursor.POS(74, 3) +"                 ")
        print(Cursor.POS(83, 3) +"                 ")
        if aux > 0:
          if not peticion_procesos_suspendidos:
            print(Cursor.POS(25, 3)+ Fore.MAGENTA + Style.BRIGHT +"ID: " + str(all_process[iteraciones].id) + Style.RESET_ALL)
            print(Cursor.POS(34, 3)+ Fore.MAGENTA + Style.BRIGHT +"Size: " + str(all_process[iteraciones].size) + Style.RESET_ALL)
          else:
        ...
        if procesos_suspendidos > 0:
          if peticion_procesos_suspendidos:
            print(Cursor.POS(74, 3)+ Fore.MAGENTA + Style.BRIGHT +"ID: "+ str(siguiente_proceso) + Style.RESET_ALL)
            print(Cursor.POS(83, 3)+ Fore.MAGENTA + Style.BRIGHT +"Size: " + str(siguiente_size) + Style.RESET_ALL)
          else:
            print(Cursor.POS(74, 3) +"ID:",siguiente_proceso)
            print(Cursor.POS(83, 3) +"Size:",siguiente_size)
        else:
          print(Cursor.POS(74, 3) +"ID: 0")
          print(Cursor.POS(83, 3) +"Size: 0")
        ...
        if estado == 'n':
          ...
          mostrar = "ejecucion"
          system("cls")
          BCP(all_process_finish,memoria,bloqueados,suspendidos,all_process,null_process,contador,mostrar,iteraciones)
          a = ''
          animacion()
          posTerm = 0
          for process in all_process_finish:
          ...
          a = ''
          if len(bloqueados) > 0:
            procesos_suspendidos += 1
            for pagina in paginas:#Cambia el estado de las paginas ocupadas por el proceso a Libre
              if pagina.proceso == bloqueados[0].id:
                pagina.ocupado = 0
                pagina.proceso = -1
                pagina.estado = "Libre"
            paginas_disponibles = paginas_disponibles + bloqueados[0].num_paginas_ocupadas
            #Redondeo de tiempos
            bloqueados[0].tiempo_bloqueado = 0
            bloqueados[0].tiempo_transcurrido = round(bloqueados[0].tiempo_transcurrido,1)
            bloqueados[0].tiempo_llegada = round(bloqueados[0].tiempo_llegada,1)
            bloqueados[0].tiempo_respuesta = round(bloqueados[0].tiempo_respuesta,1)
            string = guardar_informacion(bloqueados[0])
            with open(nombre_archivo, 'a') as archivo:
              archivo.write(string)
            suspendidos.append(bloqueados.pop(0))
            if peticion_procesos_suspendidos:
              with open(nombre_archivo, 'r') as archivo:
                lineas = archivo.readlines()
              informacion_suspendido = [""] * 15#Lista que almacena los valores del proceso suspendido
              ...
              for i in range(15):
                for letra in range(aux_informacion_suspendido,len(lineas[0])):
                  ...
                  if lineas[0][letra] != " " and lineas[0][letra] != "\n":
                    ...
                    break
              if paginas_disponibles >= int(informacion_suspendido[14]):
                peticion_procesos_suspendidos = False
                #Borrado del proceso en el txt
                lineas = lineas[1:]
                # Escribe las líneas restantes en el archivo
                with open(nombre_archivo, 'w') as archivo:
                    archivo.writelines(lineas)
                proceso = recrea_proceso(informacion_suspendido)
                _,paginas,paginas_disponibles=asignar_paginas_aProcesos(proceso,paginas,paginas_disponibles)
                memoria.append(proceso)
                procesos_suspendidos -= 1
                suspendidos.pop(0)
              else:
                peticion_procesos_suspendidos = True
                break
            if aux > 0:
              #Comprobado una vez terminado un proceso y liberado su espacio, cuantos procesos caben
              while (aux > 0):
                proceso_cupo_memoria,paginas,paginas_disponibles=asignar_paginas_aProcesos(all_process[iteraciones],paginas,paginas_disponibles)
                if proceso_cupo_memoria:
                  memoria.append(all_process[iteraciones])
                  memoria[-1].tiempo_llegada = contador
                  iteraciones += 1
                  aux -= 1
          ...
          break
    #Impresion de procesos suspendidos y siguientes
    if procesos_suspendidos > 0:
      with open(nombre_archivo, 'r') as archivo:
        lineas = archivo.readlines()
      informacion_suspendido = [""] * 15#Lista que almacena los valores del proceso suspendido
  ...
  BCP(all_process_finish,memoria,bloqueados,suspendidos,all_process,null_process,contador,mostrar,0)
  with open(nombre_archivo, 'r+') as archivo:
    archivo.truncate(0)
